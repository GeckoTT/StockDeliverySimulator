...
using MosaicDependency.Convertors;
...
{
    /// <summary>
    /// Class which implements a stream which allows to read and write objects from
    ...
    public class XmlObjectStream : IDisposable
    {
        ...
        /// <summary>
        /// If this flag is enabled, all received messages are immediately sent back (Singapur extension).
        /// </summary>
        public bool EnableMessageEcho { get; set; }
        ...
        public bool WriteRaw(byte[] message)
        {
            return _stream.Write(message, message.Length);
        }
            ...
            try
            {
                lock (_readSerializerMap)
                {
                    ...
                    do
                    {
                        ...
                        if (this.EnableMessageEcho)
                        {                            
                            if (_stream.Write(message, message.Length) == false)
                            {
                                this.Error("Sending message echo failed.");
                                return null;
                            }
                        }
