namespace CareFusion.Lib.StorageSystem.Xml
{
    ...
    internal static class ByteArrayExtensions
    {
        ...
        /// <param name="startSearchIndex">The index to start the search.</param>
        /// <returns>Zero based index if the array has been found;<c>-1</c> otherwise.</returns>
        public static int GetIndexOf(this byte[] sourceArray, byte[] searchArray, int startSearchIndex = 0)
        {
            int result = -1;

            for (int i = startSearchIndex, max = sourceArray.Length - searchArray.Length; i <= max; ++i)
            {
                result = i;
