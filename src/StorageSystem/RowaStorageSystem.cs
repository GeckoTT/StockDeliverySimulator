...
using CareFusion.Lib.StorageSystem.Wwks2.Messages.Unprocessed;
...
namespace CareFusion.Lib.StorageSystem
{
    ...
    public class RowaStorageSystem : AsyncEventProvider, IStorageSystem
    {
        ...
        private PackStoredEventHandlerEx _packStoredEx;
        private PackInputFinished _packInputFinished;
        private PackInputFinishedEx _packInputFinishedEx;
        private StockUpdatedEventHandler _stockUpdated;
        private OutputProcessFinishedEventHandler _outputProcessFinished;
        private StorageSystemArticleInfoRequestEventHandler _articleInfoRequested;
        ...
        public ComponentState State
        {
            get
            {
                ...
                {
                    this.Trace($"Storage System current state: {ComponentState.NotConnected}");
                ...
                this.Trace($"Sending StatusRequest to Storage System and waiting for a StatusResponse.");
                ...
                this.Trace(
                    $"Storage System current state: {TypeConverter.ConvertEnum(statusResponse.State, ComponentState.NotReady)}");
                return TypeConverter.ConvertEnum(statusResponse.State, ComponentState.NotReady);
            }
        }
        ...
        /// Event which is raised whenever a new pack was successfully stored in a storage system. 
        /// </summary>
        public event PackStoredEventHandlerEx PackStoredEx
        {
            add
            {
                lock (_syncLock)
        ...
        /// Event which is raised whenever a connected storage system finished an input.
        /// </summary>
        public event PackInputFinishedEx PackInputFinishedEx
        {
            add
            {
                lock (_syncLock)
        ...
        /// </summary>
        public event StorageSystemArticleInfoRequestEventHandler ArticleInfoRequested
        {
            add
            {
                lock (_syncLock)
        ...
        public void Connect(string host, ushort port = 6050)
        {
            ...
            lock (_syncLock)
            {
                ...
                _messageObjectStream.AddSupportedType(typeof(UnprocessedMessageEnvelope), typeof(UnprocessedMessage).Name);
        ...
        public void Disconnect()
        {
            lock (_syncLock)
            {
                ...
                if (_client.Connected)
                {
                    try
                    {
                        this.Trace("Shutting down the connected TCP client");
                        _client.Client.Shutdown(SocketShutdown.Both);
                        this.Trace("The TCP client is shutdown");
                    }
                    catch (Exception ex)
                    {
                                                  ...
                                                  bool expiryTracking = false)
        {
            ...
            return new Article()
            {
                ...
                StockLocationId = TextConverter.EscapeInvalidXmlChars(stockLocationId),
                MachineLocation = TextConverter.EscapeInvalidXmlChars(machineLocation),
                VirtualId = TextConverter.EscapeInvalidXmlChars(virtualArticleId),
                VirtualName = TextConverter.EscapeInvalidXmlChars(virtualArticleName),
                SerialTracking = serialTracking.ToString(),
                BatchTracking = batchTracking.ToString(),
                ExpiryTracking = expiryTracking.ToString()
            };
        }
        ...
        /// </returns>
        public IInitiateInputRequest CreateInitiateInputRequest(string id,
            int inputSource,
            int inputPoint = 0,
            int destination = 0,
        ...
        public IInitiateInputRequest CreateInitiateInputRequest(int id,
            ...
            bool setPickingIndicator = false)
        {
            return this.CreateInitiateInputRequest(id.ToString(), inputSource, inputPoint, destination, deliveryNumber,
                setPickingIndicator);
        }
        ...
        private void ProcessInputMessage(InputMessage message)
        {
            ...
            if (articles.Count > 0)
            {
                ...
                base.Raise("PackStoredEx", _packStoredEx, this, articles.ToArray(), Convert.ToBoolean(message.IsNewDelivery));
            }       
            ...
            if (_packInputFinishedEx != null)
            {
                base.Raise("PackInputFinishedEx",
                    _packInputFinishedEx,
                    this,
                    message.Source,
        ...
        /// <summary>
        /// Processes an Article Information request message.
        /// </summary>
        /// <param name="request">The request message to process.</param>
        ...
        private void ConnectionCheck(object parameter)
        {
            ...
            {                                
                ...
                if (this.EnableAutomaticStateObservation)
                {
                    if (_stateChanged != null)
                    {
                        ...
                        this.Trace(
                            $"StateChanged event is raised. The state of Storage System changes to: {currentState}.");
                    ...
                    if (_componentStateChanged != null)
                    {
                        ...
                        {
                            this.Trace(
                                $"ComponentStateChanged event is raised. The state of Storage System component {component.Type} changes to: {component.State}.");
                        ...
                        if ((DateTime.UtcNow - stateTime).TotalMilliseconds >= StateCheckInterval)
                        {
                            ...
                            if (_componentStateChanged != null)
                            {
                                for (int i = 0; i < lastComponents.Length; ++i)
                                {
                                    ...
                                    {
                                        this.Trace(
                                            $"ComponentStateChanged event is raised. The state of Storage System component {currentComponents[i].Type} changes to: {currentComponents[i].State}.");
            ...
            finally
            {
                ...
                try
                {
                    ...
                    {
                        this.Trace(
                            $"StateChanged event is raised. The state of Storage System changes to: {ComponentState.NotConnected}.");
        ...
        private bool InitializeXmlSerializer()
        {
            ...
            try
            {
                ...
                var v31 = new XmlSerializer(typeof(UnprocessedMessageEnvelope));
        ...
        /// <inheritdoc/>
        public void SendRawXml(byte[] message)
        {
            this.Info("Sends the raw xml");
            this._messageObjectStream.WriteRaw(message);
        }
