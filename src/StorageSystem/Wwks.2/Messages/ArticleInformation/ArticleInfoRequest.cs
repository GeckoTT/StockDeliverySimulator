...
namespace CareFusion.Lib.StorageSystem.Wwks2.Messages.ArticleInformation
{
    ...
    /// </summary>
    public class ArticleInfoRequest : MessageBase, IArticleInfoRequest, IStorageSystemArticleInfoRequest
    {
        #region Members
        ...
        #region IStorageSystemArticleInfoRequest Specific Properties

        /// <summary>
        /// Gets the list of articles for which general information is being requested.
        /// </summary>
        IArticleInfo[] IStorageSystemArticleInfoRequest.ArticleList
        {
            get
            {
                return (this.Article != null) ? (IArticleInfo[])this.Article : new IArticleInfo[0];
            }
        }

        #endregion

        #region IStorageSystemArticleInfoRequest Specific Methods

        /// <summary>
        /// Finishes the article information request by sending the corresponding response to the storage system.
        /// This method has to be called after the article information has been added to the articles.
        /// </summary>
        void IStorageSystemArticleInfoRequest.Finish()
        {
            if (!_isFinished)
            {
                var response = new ArticleInfoResponseEnvelope()
                ...
                if (!this.MessageObjectStream.Write(response))
                {
                    throw new ApplicationException("Sending 'ArticleInfoResponse' to storage system failed.");
                }
