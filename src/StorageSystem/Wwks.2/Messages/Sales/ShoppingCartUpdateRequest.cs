...
namespace CareFusion.Lib.StorageSystem.Wwks2.Messages.Sales
{
    ...
    public class ShoppingCartUpdateRequest : MessageBase, IShoppingCartUpdateRequest
    {
        ...
        {
            SendResponse(shoppingCart, UpdateResultStatus.NotUpdated, description,  false);
        }
        ...
        private void SendResponse(IShoppingCart shoppingCart, UpdateResultStatus status, string description, bool accepted = true)
        {
            if (!_isFinished)
            {
                ...
                if (!accepted)
                {
                    response.ShoppingCartUpdateResponse.ShoppingCart.ShoppingCartItem.RemoveAt(response.ShoppingCartUpdateResponse.ShoppingCart.ShoppingCartItem.Count - 1);
                }
