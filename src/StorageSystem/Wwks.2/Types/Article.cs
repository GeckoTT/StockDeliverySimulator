...
namespace CareFusion.Lib.StorageSystem.Wwks2.Types
{
    ...
    /// </summary>
    public class Article : IMasterArticle, IArticle, IStockDeliveryItem, IDigitalShelfArticle, IArticleStockRequest, IArticleInfo
    {
        ...
        /// <summary>
        /// Gets a flag whether batch tracking is set or not.
        /// </summary>
        [XmlIgnore]
        bool IMasterArticle.BatchTracking
        {
            get { return TypeConverter.ConvertBool(this.BatchTracking); }
        }

        /// <summary>
        /// Gets a flag whether serial number tracking is set or not.
        /// </summary>
        [XmlIgnore]
        bool IMasterArticle.SerialTracking
        {
            get { return TypeConverter.ConvertBool(this.SerialTracking); }
        }

        /// <summary>
        /// Gets a flag whether expiry date tracking is set or not.
        /// </summary>
        [XmlIgnore]
        bool IMasterArticle.ExpiryTracking
        {
            get { return TypeConverter.ConvertBool(this.ExpiryTracking); }
        }
        ...
        void IDigitalShelfArticle.AddPriceInformation(PriceCategory category, decimal price, uint quantity, decimal basePrice, string basePriceUnit, decimal vat, string description)
        {
        ...
        #region IArticleInfo Specific Properties

        /// <summary>
        /// Gets the unique identifier of the article (e.g. PZN or EAN).
        /// </summary>
        [XmlIgnore]
        string IArticleInfo.Id
        {
            get
            {
                return TextConverter.UnescapeInvalidXmlChars(this.Id);
        ...
        /// </summary>
        [XmlIgnore]
        int IArticleInfo.Depth
        {
            get
            {
                return TypeConverter.ConvertInt(this.Depth);
            }
        }
        ...
        /// </summary>
        [XmlIgnore]
        int IArticleInfo.Width
        {
            get
            {
                return TypeConverter.ConvertInt(this.Width);
            }
        }
        ...
        /// </summary>
        [XmlIgnore]
        int IArticleInfo.Height
        {
            get
            {
                return TypeConverter.ConvertInt(this.Height);
            }
        }
        ...
        /// </summary>
        [XmlIgnore]
        int IArticleInfo.Weight
        {
            get
            {
                return TypeConverter.ConvertInt(this.Weight);
            }
        }
        ...
        void IArticleInfo.SetArticleInformation(string articleId,
                                   string articleName,
                                   string dosageForm,
                                   string packagingUnit,
        ...
        {
            this.Id = TextConverter.EscapeInvalidXmlChars(articleId);
            this.Name = string.IsNullOrEmpty(articleName) ? string.Empty : TextConverter.EscapeInvalidXmlChars(articleName);
            this.DosageForm = string.IsNullOrEmpty(dosageForm) ? string.Empty : TextConverter.EscapeInvalidXmlChars(dosageForm);
            this.PackagingUnit = string.IsNullOrEmpty(packagingUnit) ? string.Empty : TextConverter.EscapeInvalidXmlChars(packagingUnit);
            this.MaxSubItemQuantity = maxSubItemQuantity.ToString();
            this.VirtualId = string.IsNullOrEmpty(virtualArticleId) ? string.Empty : TextConverter.EscapeInvalidXmlChars(virtualArticleId);
            this.VirtualName = string.IsNullOrEmpty(virtualArticleName) ? string.Empty : TextConverter.EscapeInvalidXmlChars(virtualArticleName);
            this.RequiresFridge = requiresFridge.ToString();
            
            if (serialNumberSinceExpiryDate.HasValue)
            {
                this.SerialNumberSinceExpiryDate = TypeConverter.ConvertDate(serialNumberSinceExpiryDate.Value);
            }

            this.ProductCode = new List<ProductCode>();
            foreach(string productCode in productCodeList)
            {
                this.ProductCode.Add(new ProductCode() { Code = productCode });
            }
